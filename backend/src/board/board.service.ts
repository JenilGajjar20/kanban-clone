...
export class BoardService {
  ...
  async isUserAssociatedWithBoard(boardId: number, userId: number) {
    ...
    if (count === 0) {
      throw new UnauthorizedException(
        'User is not associated with the Board!!',
      );
    }
  ...
  async create(createBoardDto: CreateBoardDto, userId: number) {
    ...
    new_board.name = createBoardDto.name;

    const user = await this.userService.findOne(userId);
    new_board.users = [user];

    return this.boardRepository.save(new_board);
  }

  findBoardsByUserId(userId: number) {
